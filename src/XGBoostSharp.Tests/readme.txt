...
- Right click XGBoostSharp project -> properties > Package, edit details (such as incrementing version number)
...
- build the package with the `/c/dev/tools/nuget.exe pack XGBoostSharp/XGBoostSharp.nuspec` command
- sign in to NuGet.org and click upload

## testing the new nuget package.
- Open the XGBoostSharpTests.sln solution (not the XGBoostSharp.sln as this does not use the NuGet package)
- Wait until the new package is indexed
- Open the Package Manager Console and type `Install-Package PicNet.XGBoostSharp`
- Build
- Run tests
...
- Add in a package source, pointing to the packed .nupkg file. Should be in XGBoostSharp\bin\Release